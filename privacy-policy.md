...
用于连接外接变率报警器（BLE 设备）。扫描与连接信息仅用于设备配对，不上传。本应用声明 `BLUETOOTH_SCAN` 权限**不用于推断位置**（`neverForLocation`）。
...
With your permission we read your device location to (a) fetch wind and weather forecasts for your flying site, (b) show your position on the map, and (c) record flight tracks and compute glide ratio. Latitude/longitude are sent over HTTPS to **Open-Meteo** (`api.open-meteo.com`) solely to retrieve weather data — no identity or device identifiers are included. Flight tracks are stored in a local on-device database, which may be included in your Android system backup managed by Google (we cannot access it). Location permission is optional; without it you can still add and select flying sites manually.

**2. Sensor data (barometer, accelerometer) — stays on device.**
Used for vario computation and takeoff/landing detection. Processed in real time, never stored or transmitted.

**3. Bluetooth — stays on device.**
Used only to pair with external BLE vario devices. The `BLUETOOTH_SCAN` permission is declared `neverForLocation` and is not used to derive location.

**4. Camera — stays on device.**
...
**6. Device identifier — stays on device.**
...
Flight logs and site configurations remain on your device until you delete them in-app or uninstall the app. Sensor data is never stored. We operate no server, so no data is retained by us.

## Your Rights

Withdraw location permission in system Settings; delete flight records in-app; uninstall to erase all local data; manage subscriptions via Google Play.
